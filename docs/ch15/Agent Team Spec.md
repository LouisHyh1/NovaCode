...
- 已有的 `worktree.Manager` 已支持嵌套 slug(`team/alice` → `.mewcode/worktrees/team+alice/`),本章直接复用做队员 worktree(slug 形式 `team-<team_name>/<member>`)
- 已有的 session 持久化(`.mewcode/sessions/<id>/conversation.jsonl`)按对话粒度落盘;本章给每个队员单独申请一个 session,队员 stop 不删 session,SendMessage 续派时通过 session 反序列化 Conversation
- 已有的 `mewcode.command` slash 命令系统,本章新增 `/team` 系列
...
- **G1**: 提供 `team.Team` 与 `team.Manager`——Team 封装小组生命周期(name、lead_agent_id、members、config_path);Manager 在单 mewcode 进程内管理多个 Team(典型场景同时只有一个活跃 Team)
...
- **F1**: `team.Team` 字段——`name`(原始名)、`sanitized_name`(经 `sanitize` 处理后用于路径)、`description`(团队描述,随 config.json 落盘)、`lead_agent_id`、`members: list[TeammateInfo]`、`config_dir`(`<home_dir>/.mewcode/teams/<sanitized_name>/`)、`config_path`(`<config_dir>/config.json`)、`created_at: datetime`、`backend: BackendType`
...
- **F15**: `mewcode.team.backend.tmux.TmuxBackend` 实现 `Backend` Protocol
...
- **F17**: `mewcode.team.backend.iterm2.Iterm2Backend` 实现 `Backend` Protocol
...
- **F18**: `mewcode.team.backend.inprocess.InProcessBackend` 实现 `Backend` Protocol
...
- **F19a**: `python -m mewcode --team-member` 在 Pane 后端被 spawn 的 mewcode 子进程**不启动 TUI**(不构造 Textual App),而是跑一个自治协程(`src/mewcode/cli/team_member.py` 的 `run_team_member`):
...
- **F63**: `~/.mewcode/teams/<sanitized_name>/config.json` 结构:
  ```json
  {
    "name": "...",
    "sanitized_name": "...",
...
- **F65**: 队员 session 沿用已有的 session 持久化机制,路径 `<project_root>/.mewcode/sessions/<id>/conversation.jsonl`;Team 删除时一并删除
...
- **AC1**: `team.Manager(...)` 在 `~/.mewcode/teams/` 不存在时自动创建;已有时正确扫描子目录还原 `teams` dict
...
- **AC7**: `Agent` 工具带 `team_name="<existing>"` 时,在 `.mewcode/worktrees/team-<sanitized>+<member>/` 落地 Worktree、调对应 backend.spawn 并在 `team.members` 里出现该成员;不带 `team_name` 时维持原行为
