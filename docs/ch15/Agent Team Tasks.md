...
## T1: 基础类型 — `src/mewcode/team/types.py`

**文件:** `src/mewcode/team/types.py`
**依赖:** 无
**步骤:**
...
## T2: sanitize 与原子写 — `src/mewcode/team/persistence.py`

**文件:** `src/mewcode/team/persistence.py`
**依赖:** T1
**步骤:**
...
## T3: Manager 与持久化 — `src/mewcode/team/manager.py`

**文件:** `src/mewcode/team/manager.py`
**依赖:** T1, T2
**步骤:**
...
## T3b: Team.Manager 跨进程并发兜底 — 继续 `src/mewcode/team/manager.py` + `src/mewcode/team/persistence.py`

**文件:** `src/mewcode/team/manager.py`(为 add_member / set_member_active 加 reload-before-modify)、`src/mewcode/team/persistence.py`(增加 `reload_from_disk_locked`)
**依赖:** T3
**步骤:**
...
## T4: Team 成员操作 — 继续 `src/mewcode/team/manager.py`

**文件:** `src/mewcode/team/manager.py`(同 T3)
**依赖:** T3
**步骤:**
...
## T5: mailbox 文件锁 — `src/mewcode/team/mailbox/lock.py`

**文件:** `src/mewcode/team/mailbox/lock.py`
**依赖:** 无
**步骤:**
...
## T6: mailbox Message 与 Box — `src/mewcode/team/mailbox/__init__.py` + `message.py`

**文件:** `src/mewcode/team/mailbox/__init__.py`、`src/mewcode/team/mailbox/message.py`
**依赖:** T5
**步骤:**
...
## T7: AgentNameRegistry — `src/mewcode/team/registry/__init__.py`

**文件:** `src/mewcode/team/registry/__init__.py`
**依赖:** 无
**步骤:**
...
## T8: tasks Store — `src/mewcode/team/tasks/__init__.py`

**文件:** `src/mewcode/team/tasks/__init__.py`
...
**文件:** `src/mewcode/team/filelock.py`(把 T5 实现迁过来)
**依赖:** 无
**步骤:**
...
## T10: backend Protocol — `src/mewcode/team/backend/__init__.py`

**文件:** `src/mewcode/team/backend/__init__.py`
**依赖:** T1
**步骤:**
...
## T11: detect_backend — `src/mewcode/team/backend/detect.py`

**文件:** `src/mewcode/team/backend/detect.py`
**依赖:** T10
**步骤:**
...
## T12: tmux backend — `src/mewcode/team/backend/tmux.py`

**文件:** `src/mewcode/team/backend/tmux.py`
**依赖:** T10
**步骤:**
...
## T13: iterm2 backend — `src/mewcode/team/backend/iterm2.py`

**文件:** `src/mewcode/team/backend/iterm2.py`
**依赖:** T10
**步骤:**
...
## T14: in-process backend — `src/mewcode/team/backend/inprocess.py`

**文件:** `src/mewcode/team/backend/inprocess.py`
...
## T15: feature flag — `src/mewcode/team/feature.py`

**文件:** `src/mewcode/team/feature.py`
**依赖:** 无
**步骤:**
...
## T16: TeammateContext — `src/mewcode/agent/team_hook.py`

**文件:** `src/mewcode/agent/team_hook.py`
**依赖:** 无
**步骤:**
...
## T17: 队员专属工具白名单 — `src/mewcode/tool/filter.py` 扩展

**文件:** `src/mewcode/tool/filter.py`(修改)
**依赖:** 无
**步骤:**
1. 新增常量:
   ```python
...
## T18: spawn_teammate 主流程 — `src/mewcode/team/spawn.py`

**文件:** `src/mewcode/team/spawn.py`
**依赖:** T1-T17
**步骤:**
...
4. 提供 helper `team_system_prompt_suffix() -> str` 返回 F39 附录

**验证:** 单测覆盖 in-process 后端的 spawn 全流程;Pane 后端的 spawn 用 mock backend

## T19: Agent 工具集成 — `src/mewcode/agent/agent_tool.py` 修改

**文件:** `src/mewcode/agent/agent_tool.py`(修改)
**依赖:** T16, T18
**步骤:**
...
## T20: 队员 Loop incoming-messages 注入 — `src/mewcode/agent/team_mailbox.py`

**文件:** `src/mewcode/agent/team_mailbox.py`
**依赖:** T16, T6
**步骤:**
...
## T21: task.Manager 改造 — `src/mewcode/task/manager.py` 修改

**文件:** `src/mewcode/task/manager.py`(修改)
**依赖:** T7
**步骤:**
...
## T22: 协作工具实现 — `src/mewcode/team/tools/`
...
## T24: coordinator 包 — `src/mewcode/coordinator/__init__.py`

**文件:** `src/mewcode/coordinator/__init__.py`
**依赖:** 无
**步骤:**
...
## T25: config 加 features — `src/mewcode/config/__init__.py` 修改

**文件:** `src/mewcode/config/__init__.py`(修改)
**依赖:** 无
**步骤:**
...
## T26: TUI 集成 — `src/mewcode/tui/app.py` 修改

**文件:** `src/mewcode/tui/app.py` 与可能的 view 文件(修改)
**依赖:** T3, T24
**步骤:**
...
## T27: /team slash 命令 — `src/mewcode/command/builtin_team.py`

**文件:** `src/mewcode/command/builtin_team.py`
**依赖:** T3
**步骤:**
...
## T28: cli wire — `src/mewcode/cli/__init__.py` 修改

**文件:** `src/mewcode/cli/__init__.py`(修改)
**依赖:** T1-T27
**步骤:**
...
## T29: --team-member 自治循环 — `src/mewcode/cli/team_member.py`(新文件)

**文件:** `src/mewcode/cli/team_member.py`(新建)
**依赖:** T28
**步骤:**
...
**文件:** `src/mewcode/cli/__init__.py`(修改)+ `src/mewcode/team/manager.py`(加 helper)
**依赖:** T21, T3
**步骤:**
...
## T30b: Lead mailbox 轮询 + 自动唤醒 — `src/mewcode/team/manager.py` + `src/mewcode/tui/tasks.py` + `src/mewcode/tui/app.py` + `src/mewcode/tui/stream.py`
...
## T31: 续写检测 — `src/mewcode/team/tools/send_message.py`

**文件:** `src/mewcode/team/tools/send_message.py`(同 T22)
**依赖:** T22, T21
**步骤:**
...
## T32: Plan 审批权限切换 — `src/mewcode/agent/team_mailbox.py`

**文件:** `src/mewcode/agent/team_mailbox.py`(修改)
**依赖:** T20
**步骤:**
