...
本章引入 `mewcode.team` 顶层包,把 SubAgent 的「子 Agent」扩展为「Team 队员」。整体分四层:
...
| 决策点                                                | 选择                                                         | 理由                                                         |
| ----------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| Team 包归属                                           | `mewcode.team` 顶层包                                        | 与 `subagent`、`worktree` 平级,职责清晰                      |
...
| Member session_dir                                    | 沿用已有的 `<root>/.mewcode/sessions/<id>/` 格式             | 复用 `session.Writer`,无需新机制;Team 删除时一并清理         |
