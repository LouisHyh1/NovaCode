...
- [ ] `uv run python -m mewcode`(或装好的 `mewcode` 入口)启动 TUI;启动消息显示一切正常,无本章相关 error
...
- [ ] 等待 alice 任务自然结束(或在 TUI 输入 `/team kill alice` 终止)
  ...
  - 验证 b:Lead 的 mailbox(`cat ~/.mewcode/teams/demo/mailbox/lead.json`)含一条正文带 `[idle]` 的文本消息,且 1-2 秒后该消息 `read=true`(watcher 已消费)
