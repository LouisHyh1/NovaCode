...
> 顺序执行。每完成一个任务跑 `ruff check mewcode/skills mewcode/tools/load_skill.py` 与 `pytest tests/test_skills.py -q` 确保通过；接入主流程的任务（T8、T9、T10）做完后立刻补一次端到端验证再进下一项。
...
- 完成标准:
  ...
  - `mewcode/prompts.py` 的 `build_environment_context` 拼接：先写 `skill_catalog` 段落，再写 `## Active Skills` 标题 + `### Skill: \n` 子段
...
- 完成标准:
  ...
  - 创建测试 skill 目录 `.mewcode/skills/test-skill/SKILL.md`，写一个简单的 inline SOP（如 `name: test-skill / description: A test skill / mode: inline`，body 写 `Echo hello`）
  - 在仓库根目录手动启动 `python -m mewcode`：
    1. `/help` 列出 `/test-skill`、`/skill` 命令
    2. `/test-skill` 能加载对应 SOP
    3. 编辑 `.mewcode/skills/test-skill/SKILL.md` 改一行后**不重启**再 `/test-skill`，新行进入 prompt（热重载验证）
...
- 影响文件: `mewcode/skills/install.py`（新建）、`mewcode/tools/install_skill.py`（新建）、`mewcode/app.py`（修改）
- 依赖任务: T9（命令注册）
- 完成标准:
