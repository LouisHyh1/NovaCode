...
def resolve_path(p: str) -> str:
    ...
    pp = Path(p)
    if pp.is_absolute():
        # 隔离上下文中:Worktree 内路径原样返回,父目录内路径映射到 Worktree。
        return remap_parent_path_if_needed(pp)
...
- `agent_tool.py`:
  ...
  - `AgentArgs` / Schema 增加可选 `isolation:"worktree"`
  - `execute` 内调用参数或 Definition 指定 `isolation == "worktree"` 时走 `self._execute_with_worktree(...)`
...
AgentTool.execute
  ↓
args.isolation == "worktree" or definition.isolation == "worktree"?
  ↓ yes
_execute_with_worktree:
  ...
  4. task_text = notice + "\n\n" + prompt
  5. with with_cwd(wt.path, parent_cwd=parent_cwd):
