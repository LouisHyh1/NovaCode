...
- [ ] `Agent` 工具 Schema 含可选 `isolation:"worktree"`,单次调用可让 `general-purpose` 进入 worktree 分支(验证:同上)
...
- [ ] SubAgent 使用父仓库绝对路径时映射到 Worktree 对应文件;已是 Worktree 绝对路径时不二次映射(验证:test_tool_ctx + test_agent_worktree)
