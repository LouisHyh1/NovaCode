...
- **G12**: 在 `subagent.Definition` 和 `Agent` 工具参数中增加 `isolation` 字段;定义默认值或单次调用指定 `isolation:worktree` 后,自动 `create → inject worktree notice → set ctx cwd → run_to_completion → auto_cleanup`
...
- **G14**: 与 ch04~ch13 协同——主 Agent 看到的工具列表不变,6 个核心文件工具不暴露 cwd 参数;`Agent` 仅增加可选 `isolation` 参数,既有调用保持兼容
...
- **F16**: 在 `mewcode.tool` 包定义 ctx key 与帮助函数——
  - Python 用 `contextvars.ContextVar("cwd", default=None)` 实现 ctx 传递(也可用显式参数,本期统一用 ContextVar 与现有 conv/depth 对齐)
  - `with_cwd(dir: str, parent_cwd: str | None = None)` 返回 context manager 设置 ContextVar token;隔离执行时同时记录父目录
  - `cwd_from_ctx() -> str | None` 取回
  - `resolve_path(p: str) -> str`——相对路径用 ctx cwd(优先)或进程 cwd 拼接;隔离执行中,父目录内的绝对路径映射到 Worktree 对应路径,已位于 Worktree 内或父目录外的绝对路径原样返回
...
- **F19**: 6 个核心文件工具 Schema 不变,ctx 注入不暴露 cwd 字段;`Agent` Schema 增加可选 `isolation:"worktree"`

### SubAgent 集成

- **F20**: 扩展 `subagent.Definition` 增加 `isolation: str` 字段;`parser.py` 解析 frontmatter `isolation:` 字段,合法值 `""` / `"worktree"`,非法值 stderr 警告后回落到 `""`
- **F21**: 改造 `agent.AgentTool.execute`——当 `args.isolation == "worktree"` 或 `definition.isolation == "worktree"` 时走 `_execute_with_worktree` 分支:
...
- **F35**: 在项目根 `.gitignore` 追加 `.mewcode/worktrees/` 与 `.mewcode/worktree_session.json` 两行;mewcode 启动时若发现 `.gitignore` 不含这两行,**只警告不修改**(尊重用户配置)
...
- **AC15**: `Agent` 单次参数或 `subagent.Definition` 指定 `isolation == "worktree"` 时,`AgentTool.execute` 创建临时 Worktree、注入 worktree notice、传 ctx cwd、跑完后调 auto_cleanup
- **AC16**: SubAgent + worktree 路径上,子 Agent即使使用父目录绝对路径写文件也不影响主 Agent 工作目录(集成测试或 tmux 实跑可观察)
