...
**文件:** `src/mewcode/worktree/slug.py` + `tests/test_worktree_slug.py`
**依赖:** 无
**步骤:**
...
**文件:** `src/mewcode/worktree/session.py`
**依赖:** T1
**步骤:**
...
**文件:** `src/mewcode/worktree/git.py` + `tests/test_worktree_git.py`
**依赖:** 无
**步骤:**
...
**文件:** `src/mewcode/worktree/manager.py` + `tests/test_worktree_manager.py`
**依赖:** T2, T3
**步骤:**
...
**文件:** `src/mewcode/worktree/create.py` + `tests/test_worktree_create.py`
**依赖:** T4
**步骤:**
...
**文件:** `src/mewcode/worktree/lifecycle.py` + `tests/test_worktree_lifecycle.py`
**依赖:** T5
**步骤:**
...
**文件:** `src/mewcode/worktree/sweep.py` + `tests/test_worktree_sweep.py`
**依赖:** T6
**步骤:**
...
**文件:** `src/mewcode/tool/ctx.py` + `tests/test_tool_ctx.py`
**依赖:** 无(并行 T1-T7)
**步骤:**
1. 在 `src/mewcode/tool/ctx.py` 定义 `_ctx_cwd: ContextVar[str | None] = ContextVar("cwd", default=None)`
2. 实现 `with_cwd(directory: str, parent_cwd: str | None = None)` 作为 `@contextmanager`;设置 cwd,隔离执行时额外记录父目录,退出时恢复两个 ContextVar token
3. 实现 `cwd_from_ctx() -> str | None`(返回 `_ctx_cwd.get()`)
4. 实现 `resolve_path(p: str) -> str`:
   - `base = _ctx_cwd.get() or str(Path.cwd())`
   - `p` 为空:返回 base
   - 绝对路径已位于 ctx cwd 或父目录外:原样返回
   - 绝对路径位于 parent_cwd:替换父目录前缀为 ctx cwd
   - 否则:返回 `str(Path(base) / p)`
5. 测试:覆盖相对路径、空路径、普通绝对路径、父目录绝对路径映射、Worktree 绝对路径不二次映射

**验证:** `pytest tests/test_tool_ctx.py -v`

## T9: 改造 6 个核心工具

**文件:** `src/mewcode/tool/{bash,read_file,write_file,edit_file,glob,grep}.py`
**依赖:** T8
**步骤:**
...
**文件:** `src/mewcode/subagent/{definition,parser}.py` + `tests/test_subagent_parser.py`
**依赖:** 无
**步骤:**
...
**文件:** `src/mewcode/agent/agent_worktree.py` + `tests/test_agent_worktree.py`
**依赖:** T6, T8, T10
**步骤:**
...
**文件:** `src/mewcode/agent/agent_tool.py`
**依赖:** T11
**步骤:**
...
3. `AgentArgs` 与工具 Schema 增加可选 `isolation:"worktree"`;在 execute 内 `args.isolation == "worktree" or definition.isolation == "worktree"` 时:
...
4. 在 inline 路径前,若有效 isolation 为 `worktree`,调 `_execute_with_worktree(self.worktree_mgr, definition, sub_agent, sub_conv, args.prompt, events)` 替代直接 `run_to_completion`
...
**文件:** `src/mewcode/command/ui.py` + `src/mewcode/command/builtin_worktree.py` + `src/mewcode/command/builtins.py` + `tests/test_command_builtins.py`
**依赖:** T6
**步骤:**
...
**文件:** `src/mewcode/tui/worktree_adapter.py` + `src/mewcode/tui/app.py`
**依赖:** T11, T13
**步骤:**
...
**文件:** `src/mewcode/cli.py` + `.gitignore`
**依赖:** T4-T14 全部
**步骤:**
