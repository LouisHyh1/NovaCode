...
- [ ] **C1**：`src/mewcode/compact/` 子包存在，可被其他模块 import。
...
- [ ] **I9**：`src/mewcode/cli.py` 启动时把 `effective_context_window(p)` 注入到 Agent。
...
- [ ] **B1**：`python -m compileall src/mewcode` 退出码 0，无语法错误；`python -c "import mewcode"` 成功。
...
- [ ] **触发**：`uv sync` 或 `pip install -e .`，在 tmux 中启动 `python -m mewcode`，配置 anthropic provider。
- [ ] **预期**：
