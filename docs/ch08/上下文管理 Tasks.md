...
| `src/mewcode/compact/state.py`            | 新建      | `ContentReplacementState`（含 `decide_once`）/ `CompactCircuitBreaker` / `RecoveryState` / `SessionContext` |
...
| `src/mewcode/conversation.py`             | 修改      | 加 `# 无需显式锁`；新增 `replace_history(new_messages)` 深拷贝整体替换 |
...
- **文件**：`src/mewcode/compact/__init__.py`、`src/mewcode/compact/const.py`
- **依赖**：无
- **步骤**：
...
- **文件**：`src/mewcode/compact/state.py`
- **依赖**：T1
- **步骤**：
...
- **验证**：`ruff check src/mewcode/compact/state.py` 干净；目测逻辑正确无并发冲突；`decide_once` 是临界区入口。
...
- **文件**：`src/mewcode/compact/token.py`
- **依赖**：T1
- **步骤**：
...
- **文件**：`src/mewcode/compact/layer1.py`
- **依赖**：T2
- **步骤**：
...
- **文件**：`src/mewcode/compact/summary_prompt.py`
- **依赖**：T1
- **步骤**：
...
- **文件**：`src/mewcode/compact/recovery.py`
- **依赖**：T4
- **步骤**：
  1. 新建 `recovery.py`。
...
- **文件**：`src/mewcode/compact/layer2.py`
- **依赖**：T5
- **步骤**：
  1. 新建 `layer2.py`。
...
- **文件**：`src/mewcode/compact/compact.py`
- **依赖**：T8、T17
- **步骤**：
  1. 新建 `compact.py`。
...
- **文件**：`src/mewcode/llm/anthropic_provider.py` / `src/mewcode/llm/openai_provider.py`
- **依赖**：T19
- **步骤**：
...
- **文件**：`src/mewcode/config/config.py` + `src/mewcode/config/protocol_defaults.py`（新建）
- **依赖**：T1
- **步骤**：
...
- **文件**：`src/mewcode/agent/runtime.py`（新建）+ `src/mewcode/agent/agent.py`（修改）
- **依赖**：T2、T3、T4、T19、T24
- **步骤**：
...
- **文件**：`src/mewcode/agent/event.py`（新建）+ `src/mewcode/agent/agent.py`、`tests/agent/test_agent.py`
- **依赖**：T27、T29
- **步骤**：
...
- **文件**：`src/mewcode/cli.py` + `scripts/smoke.py`
- **依赖**：T2、T3、T4、T26
- **步骤**：
  1. 启动阶段构造：
...
- **文件**：`src/mewcode/tui/app.py` + `src/mewcode/tui/stream.py`
- **依赖**：T26、T32
- **步骤**：
...
- **文件**：`src/mewcode/tui/stream.py`、`src/mewcode/tui/commands.py`、`tests/tui/test_tui.py`
- **依赖**：T29a、T34
- **步骤**：
