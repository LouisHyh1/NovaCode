...
| `mewcode.conversation` | compact 改 conversation | compact 拿到 `list[Message]` 后做字符串替换 / 摘要重建，再用一个新方法 `replace_history` 整体替换内存列表 |
...
依赖：`mewcode.llm.Provider`、`summary_prompt`、`recovery`、`token`、`CompactCircuitBreaker`。
...
3. **`_stream_once` 签名扩展为返回 (text, calls, usage, err)**：把现有 `(text, calls, usage, ok)` 改成 `(text, calls, usage, err: Exception | None)`。错误来源是 `StreamEvent.err`（mewcode 的 `Provider.stream` 是 async generator，错误通过事件流投递）。`_stream_once` 在收到 ev.err 时累加的 text 不写回 Conversation（保证 Conversation 状态与 stream 调用前一致，紧急压缩可以安全地 `replace_history`）。
...
`src/mewcode/conversation.py` 改动：新增 `# 无需显式锁`；新增 `replace_history(new_messages)`，做深拷贝；已有 `add_xxx` / `messages` / `length` / `last_role` 全部加锁。
...
`src/mewcode/cli.py`：启动阶段调 `new_session_context(workspace)`、`ContentReplacementState()`、`RecoveryState()`、`CompactCircuitBreaker()`，组装为 `SessionRuntime`；待 provider 选定后注入 `effective_context_window(p)`；把 `SessionRuntime` 交给 `MewCodeApp`。
