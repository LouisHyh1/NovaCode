...
### T0a: memory.Manager.list_files

**文件**：`src/mewcode/memory/manager.py`、`tests/test_memory_manager.py`
**依赖**：无
**步骤**：
...
**文件**：`src/mewcode/session/writer.py`、`tests/test_session_writer.py`
**依赖**：无
**步骤**：
...
### T0c: SessionRuntime.reset_for_new_session + tool.Registry.count

**文件**：`src/mewcode/agent/runtime.py`、`tests/test_agent_runtime.py`、`src/mewcode/tool/registry.py`
**依赖**：无
**步骤**：
...
### T1: 定义 Command 类型与 Kind 枚举

**文件**：`src/mewcode/command/command.py`
**依赖**：无
**步骤**：
...
### T2: 实现 Registry + 冲突检测 + 前缀匹配

**文件**：`src/mewcode/command/registry.py`、`tests/test_command_registry.py`
**依赖**：T1
**步骤**：
...
**文件**：`src/mewcode/command/dispatch.py`、`tests/test_command_dispatch.py`
**依赖**：无
**步骤**：
...
### T4: UI Protocol + NopUI 测试桩

**文件**：`src/mewcode/command/ui.py`
...
**文件**：`src/mewcode/command/builtin_local.py`
**依赖**：T1、T2、T4
**步骤**：
...
**文件**：`src/mewcode/command/builtin_ui.py`
**依赖**：T1、T4
**步骤**：
...
**文件**：`src/mewcode/command/builtin_prompt.py`
**依赖**：T1、T4
**步骤**：
...
### T8: register_builtins + 12 条命令一次性注册

**文件**：`src/mewcode/command/builtins.py`、`tests/test_command_builtins.py`
**依赖**：T5、T6、T7
**步骤**：
...
### T9a: TUI App 实现 UI 只读查询方法

**文件**：`src/mewcode/tui/commands.py`(在该文件已被清空旧 handler 后重写)
**依赖**：T8.5
**步骤**：
...
### T9b: TUI App 实现 UI 写入方法

**文件**：`src/mewcode/tui/commands.py`
**依赖**：T9a
**步骤**：
...
### T9c: dispatch_slash 入口 + 注册中心构造

**文件**：`src/mewcode/tui/commands.py`、`src/mewcode/tui/app.py`
**依赖**：T9b
**步骤**：
...
### T10: open_resume_menu — handle_resume 重构进 UI Protocol

**文件**：`src/mewcode/tui/resume.py`
**依赖**：T9
**步骤**：
...
**文件**：`src/mewcode/tui/complete.py`(新)
**依赖**：T2
**步骤**：
...
### T13: stream.submit 接入 dispatch_slash + view 渲染补全菜单

**文件**：`src/mewcode/tui/stream.py`、`src/mewcode/tui/view.py`
**依赖**：T12
**步骤**：
...
**文件**：`tests/test_tui.py`、`src/mewcode/prompt.py`、`tests/test_prompt.py`
**依赖**：T13
**步骤**：
