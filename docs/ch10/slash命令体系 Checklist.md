...
> 验证目标:在真实 tmux 会话内运行 mewcode 进程,按 plan/spec 描述跑完整流程。
> 准备:`uv sync`(或 `pip install -e ".[dev]"`);开一个 `~/.mewcode/memory/MEMORY.md` 保证 /memory 至少能列一条;`SESSIONS_DIR=$PWD/.mewcode/sessions`(按 mewcode 实际默认目录;如未自定义则使用 `${MEWCODE_HOME:-$PWD/.mewcode}/sessions`)。

### 场景 A: 启动与 /help

- [ ] **A1** 启动会话:`tmux new-session -d -s mewspec -x 200 -y 50 "uv run mewcode 2>/tmp/mewcode.err"`;`sleep 2`;`tmux capture-pane -t mewspec -p | grep -E "(MewCode|Send a message|DEFAULT|PLAN|default|plan)"` 至少命中 2 个关键字
...
- [ ] **E1** /review 注入审查请求:`tmux send-keys -t mewspec '/review' Enter`;`sleep 5`;capture 状态栏进入流式 + AI 开始回复;检查最新 session JSONL 文件:`tail -3 $SESSIONS_DIR/<latest>.jsonl` 含一条 `"role":"user"` 且文本含"审查"关键字
...
- [ ] **H1** 临时编辑 `src/mewcode/command/builtins.py`,把 /help 命令再注册一次同名;`python -m mewcode 2>&1 | head -10`;输出含 "RuntimeError" 与 "help"(或具体冲突文本);还原 builtins.py
